import pandas as pd
excel_path = "C:\\Users\\Acer\\OneDrive\\Desktop\\Veri Bilimi\\8. Hafta\\obs.xlsx"
...
for idx in range(len(unique_students)):
    ...
    result.at[idx, "ders_sayisi"] = ders_sayisi
...
num_cols = [
    "vize_ort", "vize_min", "vize_mak", "odev_ort", "odev_min", "odev_mak", "final_ort",
    "final_min", "final_mak", "ort_ort", "ort_min", "ort_mak",
]
result[num_cols] = result[num_cols].astype(float).round(2)
result["ders_sayisi"] = result["ders_sayisi"].astype(int)

result.to_excel("obs1-b_at-iat.xlsx", index=False)